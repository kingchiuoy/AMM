...
public class MapMatching {
    ...
    public double scoreThre = 0.4;
    public double sampleInterval = 30;
    ...
    private List<SequenceState<State, Observation, Path>> computeFastConfidenceSequence(List<ObservationWithCandidateStates> timeSteps) {
...
//        System.out.println("TimeStepSize: " + timeSteps.size());
        ...
        for (ObservationWithCandidateStates timeStep : timeSteps) {
            final Map<State, Double> emissionLogProbabilities = new HashMap<>();
//            Map<Transition<State>, Double> transitionLogProbabilities = new HashMap<>();
//            if(timeStep.observation.accuracy > 100) {
...
//              This is the distance of the Priori Probability
                ...
                double timeDis = Math.ceil((double)(timeStep.observation.getTimeStamp() - prevTimeStep.observation.getTimeStamp()));
                preTime.start();
                speedStatics.addValue(timeStep.observation.getSpeed());
...
//                The calculation of the Speed Probability.
                ...
                for(State to : timeStep.candidates) {
                    ...
                    for (State from : prevTimeStep.candidates) {
                        ...
                        if(1.0 / dist * speedProb > maxProb) {
                            maxPrev = from;
                            maxProb = 1.0 / dist * speedProb;
                        }
...
//                System.out.println(finalProbs.stream().mapToDouble(a->a).summaryStatistics());
                ...
                if (score < scoreThre)  {
//                    System.out.println("Reduce nodes");
                    System.out.println("The " + stepCounter + "point" + "is removed due to Score: " + score);
                    stepCounter++;
                ...
                while (!isBackWardFinished) {
                    ...
                    for (State from : prev.candidates) {
                        ...
                        for (State to : cur.candidates) {
                            ...
                            if (path.isFound()) {
                                double tmp = probabilities.transitionLogProbability(path.getDistance(), otherlinearDistance);
                                prob += tmp * to.getProb();
                            }
                        }
        ...
        loopWatch.stop();
        System.out.println("Loop Time: " + loopWatch.getSeconds());
        System.out.println("Pre Time: " + preTime.getSeconds());
        System.out.println("Cal Time: " + calTime.getSeconds());
        System.out.println("Back Time: " + backWardTime.getSeconds());
        System.out.println("Speed Time: " + speedTime.getSeconds());
        System.out.println("Path Time: " + pathTime.getSeconds());
        System.out.println("Dir Time: " + dirTime.getSeconds());
        System.out.println("Pos Time: " + posTime.getSeconds());
        System.out.println("After Time: " + afterTime.getSeconds());
        System.out.println("Back Time: " + backWardTime.getSeconds());
...
//        State prev = getMaxState(timeSteps.get(timeSteps.size() -  1).candidates, 1);
        ...
        buildWatch.stop();
        System.out.println("Average cand: " + candidateList.stream().mapToDouble(a->a).summaryStatistics());
        System.out.println("Build Time: " + buildWatch.getSeconds());
        return result;
    }
